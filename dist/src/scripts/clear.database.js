...
async function init() {
    ...
    try {
        const confirmed = await confirmDeletion(`WARNING: This will delete ALL ${warnText} data.`);
        if (!confirmed) {
        ...
        await db.run(sql.raw(`COMMENT ON SCHEMA public IS 'standard public schema';`));
